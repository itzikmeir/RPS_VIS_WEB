...
def _overlay_text(step: dict, idx: int, total: int, schedule: dict) -> str:
    ...
    cur_ti = step.get("ti")
    for mi, model in enumerate(schedule["models"]):
        mi_mark = "►" if mi == cur_mi else " "
        lines.append(f"{mi_mark} Model {mi+1} ({model['model_type']})")
        for vi, vis in enumerate(model["visualizations"]):
            vi_mark = "►" if (mi == cur_mi and vi == cur_vi) else " "
            n = len(vis["trials"])
            ti_str = f"  T{(cur_ti or 0)+1}/{n}" if (mi == cur_mi and vi == cur_vi and cur_ti is not None) else ""
            lines.append(f"     {vi_mark} {vis['visualization']}{ti_str}")
    return "\n".join(lines)


def inject_overlay(page, step: dict, idx: int, total: int, schedule: dict):
    text = _overlay_text(step, idx, total, schedule)
...
def run(pid: str = "P001", delay: float = 2.0):
    ...
    with sync_playwright() as pw:
        ...
        for idx, step in enumerate(steps):
            ...
            try:
                inject_overlay(page, step, idx, total, schedule)
            except Exception:
            ...
            if kind == "login":
                ...
                # Scroll to top so all fields are reachable
                page.evaluate("window.scrollTo(0,0)")
                fill_page(page, kind)
                # Scroll through the page so all fields get filled
        ...
        # Check for logged data in localStorage
        try:
